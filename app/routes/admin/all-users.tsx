...
const AllUsers = ({ loaderData }: Route.ComponentProps) => {
    ...
    return (
        <main className="all-users wrapper">
            <Header
                title="Manage Users"
                description="Filter, sort, and access user detailed user profiles"
            />
                    ...
                    <ColumnDirective
                        ...
                        template={(props: UserData) => (
                            <article className={cn('status-column', props.status === 'user' ? 'bg-success-50' : 'bg-light-300')}>
                                <div className={cn('size-1.5 rounded-full', props.status === 'user' ? 'bg-success-500' : 'bg-light-gray-500')} />
                            </article>
                        )}
                    />
                </ColumnsDirective>
